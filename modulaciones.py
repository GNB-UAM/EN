...
class Modulacion(object):
    ...
    def __init__(self,config_experiment,**kwargs):
        ...
        self.succiones = config_experiment[tc.SUCCION]
        self.switchs = config_experiment[tc.SWESTIM]
        self.samples_ini = config_experiment[tc.SINICIO]
        self.cte = config_experiment[tc.SBESTIM]
        self.name_files = config_experiment[tc.NFILE]
        self.name_folders = config_experiment[tc.NFOLDER]
        self.vecs_open_valves = config_experiment[tc.VECOPVAL]
        self.vecs_anal_odort = config_experiment[tc.VECOPODR]
        self.sleeps = config_experiment[tc.SLEEP]
        self.time_mark = datetime.now()
        self.f,self.g,self.h,self.thiread = None,None,None,None
        self.air_loop = False
        Modulacion.motorPin = config_experiment[tc.MTPORT] if config_experiment[tc.MTPORT] != '' else 'P9_21'
        Modulacion.NM = int(config_experiment[tc.NMUETS]) if config_experiment[tc.NMUETS] != '' else 10
        Modulacion.T = float(config_experiment[tc.RDTIME]) if config_experiment[tc.RDTIME] != '' else 0.1
        Modulacion.SLEEP = int(config_experiment[tc.SLEEPM]) if config_experiment[tc.SLEEPM] != '' else 1
        Modulacion.SLEEP_tyh = int(config_experiment[tc.SLEEPTYH]) if config_experiment[tc.SLEEPTYH] != '' else 59
        Modulacion.Vcc = int(config_experiment[tc.VCC]) if config_experiment[tc.VCC] != '' else 5
        Modulacion.Temp22 = config_experiment[tc.TEMPPORT] if config_experiment[tc.TEMPPORT] != '' else 'P8_11'
        Modulacion.electrovalvulas = config_experiment[tc.ELECPORTS] if config_experiment[tc.ELECPORTS] != [''] else ['P8_10','P8_12','P8_14','P8_16']
        Modulacion.sensorTemp22 = Modulacion.TyH_sensor[config_experiment[tc.SENTYPE]]
        Modulacion.tsub = Modulacion.T / Modulacion.NM
        Modulacion.electrpos = config_experiment[tc.VALPOS]
    ...
    def reset(self,heatPin):
        ...
        self.g.writelines("Experimento finalizado con exito. Fecha de finalizacion: %s\n"%(datetime.now()))
        self.g.flush()
    ...
    def handler_signal(self,signum,frame):
        self.g.writelines("Se ha recibido una senial para el cambio de muestras\n")
        self.g.flush()
...
class Puro(Modulacion):
    ...
    def __init__(self,config_experiment,**kwargs):
        ...
        Puro.Rl_2600 = int(config_experiment[tc.RSTCE]) if config_experiment[tc.RSTCE] != '' else 440
        Puro.sensorPin2600 = config_experiment[tc.RDPORT] if config_experiment[tc.RDPORT] != '' else 'P9_38'
        Puro.heatPin2600 = config_experiment[tc.HTPORT] if config_experiment[tc.HTPORT] != '' else ''
        Puro.path = config_experiment[tc.SDFOLDER] if config_experiment[tc.SDFOLDER] != '' else 'CAPTURAS/PURO'
        self.muestras = 0
    ...
    def valor_sensor(self,string,gas):
        ...
        self.event_TyH.wait()
        self.f.writelines("%d %f %f 100 %f %f %s %s\n"%(self.muestras,valueTGS2600,RsTGS2600,self.temperature,self.humidity,instante_captura,gases))
        self.f.flush()
        self.g.writelines("%s[%d] Valor(mV): %f Rs(ohmios): %f Temperatura(5V): 100 Temperatura_ambiental: %f Humedad_ambiental: %f Instante_Captura: %s Identificador_gases: %s %s\n"%
    ...
    def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio):
        self.g.writelines("Algoritmo temperatura constante\n")
        self.g.flush()
        ...
        self.g.writelines("Valor de la resistencia de carga: %d\n"%(Puro.Rl_2600))
        self.g.flush()
    ...
    def inicializar_ficheros_puertos_hilos(self,switch,samplesinicio,ct,nfile,nfolder,vsovs,vsaodrs,arg_extra=None):
        ...
        self.f.writelines("%d %d %d %d"%(switch,samplesinicio,ct,len(vsaodrs)))
        self.f.flush()
        ADC.setup()
...
class Regresion(Modulacion):
    ...
    def __init__(self,config_experiment,**kwargs):
        ...
        self.tendencia = config_experiment[tc.TEND]
        self.heat = config_experiment[tc.HTSENSOR]
        self.x=[]
        self.concentTGS2600=[]
        self.muestras = 0
        Regresion.Rl_2600 = int(config_experiment[tc.RSTCE]) if config_experiment[tc.RSTCE] != '' else 440
        Regresion.sensorPin2600 = config_experiment[tc.RDPORT] if config_experiment[tc.RDPORT] != '' else 'P9_40'
        Regresion.heatPin2600 = config_experiment[tc.HTPORT] if config_experiment[tc.HTPORT] != '' else 'P9_22'
        Regresion.path = config_experiment[tc.SDFOLDER] if config_experiment[tc.SDFOLDER] != '' else 'CAPTURAS/REGRESION'
    ...
    def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio,tendencia):
        self.g.writelines("Algoritmo temperatura variable y codificacion en amplitud\n")
        self.g.flush()
        super().file_TGS2600(ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio)
    ...
    def inicializar_ficheros_puertos_hilos(self,suc,sw,samplesinicio,ct,nfile,nfolder,vsovs,vsaodrs,arg_extra=None):
        ...
        self.f.writelines("%d %d %d %d\n"%(sw,samplesinicio,ct,len(vsaodrs)))
        self.f.flush()
...
class MPID(Modulacion): #ModulationPID
    ...
    def __init__(self,config_experiment,**kwargs):
        super().__init__(config_experiment,**kwargs)
        self.periods = config_experiment[tc.PIDPERIOD]
        self.heat = config_experiment[tc.HTSENSOR]
        self.Kp = list(np.array(config_experiment[tc.ALPHA])*0.6)
        self.Kd = list(np.array(self.periods)*0.125)
        self.Ki = list(np.array(self.periods)*0.5)
        self.temperature_Max_Upper_Bound = config_experiment[tc.MAXLIMITUPPERBOUND]
        self.temperature_Min_Upper_Bound = config_experiment[tc.MINLIMITUPPERBOUND]
        self.temperature_Max_Lower_Bound = config_experiment[tc.MAXLIMITLOWERBOUND]
        self.temperature_Min_Lower_Bound = config_experiment[tc.MINLIMITLOWERBOUND]
        self.max_peak_value = config_experiment[tc.MAXPKVAL]
        self.min_peak_value = config_experiment[tc.MINPKVAL]
        ...
        MPID.Rl_2600 = int(config_experiment[tc.RSTCE]) if config_experiment[tc.RSTCE] != '' else 440
        MPID.sensorPin2600 = config_experiment[tc.RDPORT] if config_experiment[tc.RDPORT] != '' else 'P9_40'
        MPID.heatPin2600 = config_experiment[tc.HTPORT] if config_experiment[tc.HTPORT] != '' else 'P9_22'
        MPID.path = config_experiment[tc.SDFOLDER] if config_experiment[tc.SDFOLDER] != '' else 'CAPTURAS/MPID'
    ...
    def valor_sensor(self,string, gas, subtarget, Kp, Kd, Ki, temperaturaMaxUpperBound, temperaturaMinLowerBound):
        ...
        self.event_TyH.wait()
        self.f.writelines("%d %.5f %.5f %.5f %.5f %.5f %.5f %.5f %s %s\n"%
        ...
        self.f.flush()
        self.g.writelines("%s[%d] Target(mV): %.5f Valor sensor(mV): %.5f Rs(ohmios) %.5f Temperatura: %.5f Temperatura_PID: %.5f Temperatura_ambiental: %.5f Humedad_ambiental: %.5f Instante Captura: %s Identificador_gases: %s %s\n"%
    ...
    def captura_odorante(self,vector_valvulas,vector_odorantes,n_muestras,string,periodo, Kp, Kd, Ki, 
        ...
        for count in range(n_muestras):
            ...
            if self.muestras % periodo == 0 and self.muestras != 0:
                self.g.writelines("Se procede a recalcular el target\n")
                self.g.flush()
    ...
    def inicializar_ficheros_puertos_hilos(self,suc,sw,samplesinicio,ct,nfile,nfolder,vsovs,vsaodrs,arg_extra=None):
        ...
        self.f.writelines("%d %d %d %d"%(sw,samplesinicio,ct,len(vsaodrs)))
        self.f.flush()
        ADC.setup()
    ...
    def auxiliar_capturas_muestras_iniciales(self,samplesinicio):
        ...
        for count,heat in enumerate(heatTGS):
            ...
            self.f.flush()
            self.g.writelines("Muestras_iniciales[%d] Valor(mV): %.5f Rs(ohmios) %.5f Temperatura: %.5f Instante Captura: %s Identificador_gases: %s\n"%
...
class Martinelli(Modulacion):
    ...
    def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_data2,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio,mode_execution,mode_temperature,Tmin,Tmax):
        self.f.writelines("Algoritmo temperatura variable y codificacion en frecuencia\n")
        super().file_TGS2600(ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio)
        self.f.writelines(["Nombre del fichero de datos 2: %s"%nameFile_data2,
        	"Pin PWM de calentamiento sensor: %s"%self.heatPin2600,
        	"Pin ADC sensor: %s"%self.sensorPin555,
        	"Modo de ejecucion: %s"%mode_execution,
        	"Modo de cambio de temperatura: %s"%mode_temperature,
        	"Temperatura minima del sensor: %d"%Tmin,
        	"Temperatura maxima del sensor: %d"%Tmax])
        self.f.flush()
