...
class Modulacion(object):
        ...
        def __init__(self,config_experiment,config_platform,strings,**kwargs):
                ...
                self.f,self.g,self.h,self.thread = None,None,None,None
                ...
                # PROBAR LO SIGUIENTE
                Modulacion.queue_open_loop = qu.Queue() # Creo la cola de mensaje global
                Modulacion.event_open_loop = Event() # Evento para sincronizar los hilos de captura y de escritura
                self.open_loop_thread = Thread(target=self.hilo_bucle_abierto) # Creo el hilo de escritura global
                self.open_loop_thread.start()
        ...
        def create_files(self,nameFile,path,folder,samplesinicio):
                ...
                return ruta2.strip() + str(fileString),ruta1.strip() + str(fileString_data),ruta3.strip() + str(fileString_tyh)

        def open_files(self,ruta_fichero,ruta_fichero_data,ruta_fichero_tyh):
                return os.fdopen(os.open(ruta_fichero, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o644), 'w'),os.fdopen(os.open(ruta_fichero_data, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o644), 'w'),os.fdopen(os.open(ruta_fichero_tyh, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o644), 'w')
        ...
        def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_tyh,vec_anal_odor,succion,heat2600,tiempo,switch,tespera,samplesinicio):
                self.f.write ('Configuracion de la plataforma\n')
                self.f.write ('Pin del motor: '+str(Modulacion.motorPin)+'\n')
                self.f.write ('Numero de capturas ADC para una muestra: '+str(Modulacion.NM)+'\n')
                self.f.write ('Tiempo en que se realizan las NM capturas de una muestra: '+str(Modulacion.T)+'\n')
                self.f.write ('Tiempo limite de captura de una muestra: '+str(Modulacion.SLEEP)+'\n')
                self.f.write ('Tiempo limite de captura de una muestra de TyH: '+str(Modulacion.SLEEP_tyh)+'\n')
                self.f.write ('Valor de VCC del circuito: '+str(Modulacion.Vcc)+'\n')
                self.f.write ('El puerto de lectura de la humedad y temperatura es el: '+str(Modulacion.Temp22)+'\n')
                self.f.write ('Los puertos de las electrovalvulas son: '+str(Modulacion.electrovalvulas)+'\n')
                self.f.write ('Posicion de las electrovalvulas: '+Modulacion.electrpos+'\n')
                self.f.write ('Sensor TGS2600\n')
                self.f.write ('Fecha y hora de inicio: ' + str(time.strftime("%a%d%b%Y-%HH%MM%SS", time.localtime())) + '\n')
                self.f.write ('Ruta del fichero: ' + str(ruta) + '\n')
                self.f.write ('Nombre del fichero: ' + str(nameFile) + '\n')
                self.f.write ('Nombre del fichero de datos 1: ' + str(nameFile_data1) + '\n')
                self.f.write ('Nombre del fichero de TyH: ' + str(nameFile_tyh) + '\n')
                self.f.write ('Electrovalvulas (1-METANOL, 2-ETANOL, 3-BUTANOL, 4-AIRE ) \n')
                self.f.write ('Conmutacion entre electrovalvulas: ' +str(vec_anal_odor)+ '\n')
                self.f.write ('Succion motor (30-100)   >>> '+ str(succion)+ '%\n')
                self.f.write ('Temperatura TGS2600 (1-100) >>> '+str(heat2600)+ '\n') #+' Pin PWM : ' +str(heatPin2600)
                self.f.write ('Duracion del experimento: ' +str(tiempo)+ ' minutos o muestras\n')
                self.f.write ('Se ha dejado un tiempo entre captura de gases de: '+str(tespera)+ ' segundos\n')
                self.f.write ('El experimento tiene: ' +str(samplesinicio)+ ' muestras iniciales\n')    
                self.f.write ('El tiempo de switch o conmutacion de las electrovalvulas es de: '+str(switch)+'\n')              
        ...
        def abrir_electrovalvulas(self,electrovalvulas):
                #print("Las electrovalvulas :",electrovalvulas)
                for number_elect in electrovalvulas:
                        #print ("Las electrovalvulas que se van a abrir son: ",number_elect)
                        GPIO.output(Modulacion.electrovalvulas[number_elect-1], GPIO.HIGH)
                        #print ("Activada")

        #def imprimir_electrovalvulas(self,elecs):
        #
        #       for elec in elecs:
        #               print (Modulacion.print_electrovalvulas[elec])
        ...
        def measure_tyh(self):
                ...
                while getattr(self.thread,"do_run",True):
                        tick_HT = time.time()
                        humidity, temperature = DHT.read_retry(Modulacion.sensorTemp22,Modulacion.Temp22,30,1,None)#read_retry(sensorTemp22,Temp22)
                        ...
                        if t_HT > Modulacion.SLEEP_tyh:
                                ...
                                print ('\n Sensor DHT22: ' + str(Modulacion.sensorTemp22))
        ...
        def cierre(self,heatPin):
                
                Modulacion.queue.put(0) 
                #Modulacion.queue_open_loop.put(0)
                Modulacion.event.wait() # Me sincronizo con el hilo de escritura para asegurarme de que ha tenido tiempo para escribir todo
                #Modulacion.event_open_loop.wait() # Me sincronizo con el hilo de escritura para asegurarme de que ha tenido tiempo para escribir todo
                ...
                self.f.write('Fecha y hora de fin de experimentacion:' +str(fecha_fin))
                self.f.flush()
                self.f.close()
                self.g.close()
                self.h.close()
        ...
        def hilo_escritura_datos(self,strings):
                ...
                while(1):
                        ...
                        for value,string in zip(values,strings):
                                str1+=str(value)+' '
                                str2+=string+' '+str(value)+' '
                        str2+='Gas(es) captados e identificadores: '
                        
                        #print("STR1.2",str1,"STR2.2",str2)
                        for gas in gases:
                                str1+=str(gas)+' '+Modulacion.odorantes[gas]
                                str2+=str(gas)+' '+Modulacion.odorantes[gas]
                        
                        #print("STR1.3",str1,"STR2.3",str2)
                        self.g.write(str1+'\n')
                        self.g.flush()
                        self.f.write(str2+'\n')
                        self.f.flush()
        ...
        def crear_hilo_TyH(self):
                self.thread = Thread(target=self.measure_tyh,args=())
                self.thread.do_run = True
                self.thread.start()

        #Puro.path -- > tiene que ser un path de cada uno
        def apertura_escritura_ficheros(self,path,sw,samplesinicio,ct,nfile,nfolder,vsaodrs,arg_extra=None):
                ...
                print ('Ruta Fichero de datos TyH: '+ ruta_fichero_tyh)

                return

        def inicializar_hilos_puertos(self,suc):
                Modulacion.crear_hilo_TyH(self)
                Modulacion.motor_start(self,suc)

        def capturas_muestras_iniciales(self,samplesinicio):
                print ('\n\nComienza la adquisicion. Muestras inciales: ' +str(samplesinicio)+ '\n\n')
                self.f.write('\n\nComienza la adquisicion. Muestras inciales: ' +str(samplesinicio)+ '\n\n')
                self.f.flush()
                Modulacion.cerrar_electrovalvulas(self)

        def captura_muestras(self):
                print ('\n\nComienza la experimentacion\n')
                self.f.write('\n\nComienza la experimentacion\n')
                self.f.flush()
                Modulacion.cerrar_electrovalvulas(self)

        def captura_datos(self,*args):
        ...
        #PUEDE QUE NO ESTÉ DEL TODO BIEN PUESTO
        def cierre_hilo_escritura(self):
            #Enviamos al hilo de escritura un -1, que es la senial de que finalice, y esperamos para asegurarnos de que acaba
            Modulacion.queue.put(-1)
            Modulacion.event.wait()
            
            Modulacion.queue_open_loop.put(-1)
            Modulacion.event_open_loop.wait()

            #Esperamos a que el hilo acabe
            self.write_thread.join()
            self.open_loop_thread.join()

            return
    
        # Con este hilo pretendo crear un bucle abierto que clasifique los datos, pero sin retroalimentar a la nariz
        # Variables de la clase no declaradas
        def hilo_bucle_abierto(self):
                
            open_loop = open('open_loop_classifier.txt','w')
            # Cuidado con el pickle, abrir solo datos que sean de confianza
            with open('SVM.pkl','rb') as f:
                svm = pickle.load(f)
                data_media = pickle.load(f)

            print(svm)
            print(data_media)
            while(1):
                values = Modulacion.queue_open_loop.get()
                if values == -1:
                    Modulacion.event_open_loop.set()
                    return 0
                if values == 0:
                    Modulacion.event_open_loop.set()
                    continue
                ...
                vector = []
                for mdata in data_media:
                    vector.append(np.correlate(values[0],mdata)[0])

                clase = svm.predict([vector])
                #De la clase que se le pasa solo coge el primer odorante
                open_loop.write("La clase que se predice es: "+str(clase)+" y la clase real es: "+str(values[1][0])+"\n")
...
class Puro(Modulacion):
        ...
        def valor_sensor(self,count,string,gas):
                ...
                print("Envio esto:",[string,count,valueTGS2600,RsTGS2600,instante_captura,gas])
                Modulacion.queue.put([string,count,valueTGS2600,RsTGS2600,instante_captura,gas])
        ...
        def captura_odorante(self,vector_valvulas,vector_odorantes,n_muestras,string):
                ...
                for count in range(n_muestras):
                        #time_ini = time.time()
                        value_sleep = self.valor_sensor(self.muestras,string,vector_odorantes)
                        #time_end = time.time()
        ...
        def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio):
                self.f.write ('Algoritmo temperatura constante\n')
                ...
                self.f.write('Valor de la resistencia de carga: '+str(Puro.Rl_2600)+'\n')
        
        def apertura_escritura_ficheros(self,sw,samplesinicio,ct,nfile,nfolder,vsaodrs,arg_extra=None):
                super().apertura_escritura_ficheros(Puro.path,sw,samplesinicio,ct,nfile,nfolder,vsaodrs)
                self.g.writelines(str(sw)+' '+str(samplesinicio)+' '+str(ct)+' '+str(len(vsaodrs))+'\n')

        def inicializar_hilos_puertos(self,suc,sw,samplesinicio,ct,nfile,nfolder,vsovs,vsaodrs,arg_extra=None):
...
class Regresion(Modulacion):
        ...
        def valor_sensor(self,count,temperature_TGS2600,string,opcion,gas,samplesinicio,tendencia):
                ...
                Modulacion.queue.put([string,count,valueTGS2600,RsTGS2600,temperature_TGS2600,instante_captura,slope, intercept, r_value, p_value, std_err1,gas])
                return (time_end-time_ini)
        ...
        def captura_odorante(self,vector_valvulas,vector_odorantes,n_muestras,string,opcion,heat2600,samplesinicio,tendencia):
                ...
                for count in range(n_muestras):
                        #time_ini = time.time()
                        value_sleep = self.valor_sensor(self.muestras,heat2600,string,opcion,vector_odorantes,samplesinicio,tendencia)
                        #time_end = time.time()
                        time.sleep(Modulacion.SLEEP - value_sleep)
                        self.muestras+=1
                super().cerrar_electrovalvulas()
                ############
                Modulacion.queue_open_loop.put([self.concentTGS2600[-n_muestras:],vector_odorantes])
                ###########
        ...
        def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio,tendencia):
                self.f.write('Algoritmo temperatura variable y codificacion en amplitud\n')
                super().file_TGS2600(ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio)
                self.f.write('Pin PWM de calentamiento sensor: ' + str(self.heatPin2600)+'\n')
                self.f.write('Valor de la tendencia: ' + str(tendencia)+'\n')
                self.f.write('Pin ADC sensor: ' + str(self.sensorPin2600)+'\n')
                self.f.write('Valor de la resistencia de carga: '+str(Regresion.Rl_2600)+'\n')

        def apertura_escritura_ficheros(self,sw,samplesinicio,ct,nfile,nfolder,vsaodrs,arg_extra=None):
                super().apertura_escritura_ficheros(Regresion.path,sw,samplesinicio,ct,nfile,nfolder,vsaodrs)
                self.g.writelines(str(sw)+' '+str(samplesinicio)+' '+str(ct)+' '+str(len(vsaodrs))+'\n')
                return
        ...
        def inicializar_hilos_puertos(self,suc,sw,samplesinicio,ct,nfile,nfolder,vsovs,vsaodrs,arg_extra=None):
                ...
                self.file_TGS2600(tc.obtener_ruta(Regresion.path,self.time_mark,nfolder,''),nfile+".txt",nfile+".dat","TyH"+nfile+".data",
                        vsaodrs,suc,str(arg_extra[0]*0.01*5)+"V",float(samplesinicio + (ct*(len(vsovs)+1)) + (len(vsovs)*sw)),sw,ct,samplesinicio,arg_extra[1])
...
class MPID(Modulacion): #ModulationPID
        ...
        def crear_target(self,Vmax, Vmin, periodo):
                ...
                print(max(self.target),min(self.target),self.target)
        ...
        def recalcular_target(self,temperature_Max_Upper_Bound,temperature_Min_Upper_Bound,temperature_Max_Lower_Bound,temperature_Min_Lower_Bound,period):
                ...
                print("Recreo el target con los valores: ",max_bound,min_bound,period,self.max_value,self.min_value,self.max_PID_temp,self.min_PID_temp)
        ...
        def valor_sensor(self,count,string, gas, subtarget, Kp, Kd, Ki, temperaturaMaxUpperBound, temperaturaMinLowerBound):
                """
                Captura muestras usando la tecnica de Martinelli
                Realiza la toma de medidas usando como tecnica la descrita por Martinelli
                Parametros:
                ...
                print("ARGUMENTOS PASADOS: ",count,string, gas, subtarget, self.lastError, self.addError, self.temp, Kp, Kd, Ki, temperaturaMaxUpperBound, temperaturaMinLowerBound)
                #Lectura nariz
                time_ini = time.time()
                value=0
                ADC.read(Puro.sensorPin2600) #la primera medida es erronea por el bug
                ...
                print("TemperaturePID: ",temperaturaPID,t1)
                ...
                time_end = time.time()
                Modulacion.queue.put([string,count,subtarget,valueTGS2600,RSTGS,self.temp,temperaturaPID,instante_captura,gas])
        ...
        def calculate_min_max_value(self, subtarget, valueTGS2600, func, variable, tempPID, selftempPID):
                ...
                if subtarget == func(self.target):
                        print("DENTRO BIEN: ",func,subtarget,valueTGS2600)
        ...
        def captura_odorante(self,vector_valvulas,vector_odorantes,n_muestras,string,periodo, Kp, Kd, Ki, 
                ...
                super().abrir_electrovalvulas(vector_valvulas)
                print("SELF.MUESTRAS: ",self.muestras,self.lastError,self.addError,self.temp)
                for count in range(n_muestras):

                        if self.muestras % periodo == 0 and self.muestras != 0:
                            print("Recalculo el periodo: ",temperature_Max_Upper_Bound,temperature_Min_Upper_Bound,temperature_Max_Lower_Bound,temperature_Min_Lower_Bound,periodo)
                        ...
                        #time_ini = time.time()
                        ret_values = self.valor_sensor(self.muestras,string,vector_odorantes, subtarget, Kp, Kd, Ki, temperature_Max_Upper_Bound, temperature_Min_Lower_Bound)
                        #time_end = time.time()
                        ...
                        print("HOLA CARACOLA: ",subtarget,self.max_PID_temp,self.min_PID_temp,self.max_value,self.min_value,ret_values,self.lastError,self.addError,self.temp)
                        self.muestras+=1
        ...
        def file_TGS2600(self,ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,
                ...
                self.f.write('Algoritmo modulacion por PID en lazo cerrado\n')
                super().file_TGS2600(ruta,nameFile,nameFile_data1,nameFile_tyh,vec_open_valve,succion,heat2600,tiempo,switch,tespera,samplesinicio)
                self.f.write('Valor de la resistencia de carga: '+str(MPID.Rl_2600)+'\n')
                self.f.write('Control Proporcional: '+str(Kp)+'\n')
                self.f.write('Control Integral: '+str(Ki)+'\n')
                self.f.write('Control Derivativo: '+str(Kd)+'\n')
                self.f.write('Pin PWM de calentamiento sensor: ' + str(self.heatPin2600)+ '\n')
                self.f.write('Pin ADC sensor: ' + str(self.sensorPin2600) + '\n')
                self.f.write('Periodo de la senial: '+str(period)+'\n')
                self.f.write('Limite maximo superior: '+str(temperature_Max_Upper_Bound)+'\n')
                self.f.write('Limite minimo superior: '+str(temperature_Min_Upper_Bound)+'\n')
                self.f.write('Limite maximo inferior: '+str(temperature_Max_Lower_Bound)+'\n')
                self.f.write('Limite minimo inferior: '+str(temperature_Min_Lower_Bound)+'\n')
                self.f.write('Valor pico inicial maximo: '+str(maximum_peak_value)+ '\n')
                self.f.write('Valor pico inicial minimo: '+str(minimum_peak_value)+ '\n')
        
        def apertura_escritura_ficheros(self,sw,samplesinicio,ct,nfile,nfolder,vsaodrs,arg_extra=None):
                super().apertura_escritura_ficheros(MPID.path,sw,samplesinicio,ct,nfile,nfolder,vsaodrs)
                self.g.writelines(str(sw)+' '+str(samplesinicio)+' '+str(ct)+' '+str(len(vsaodrs))+'\n')
        ...
        def capturas_muestras_iniciales(self,samplesinicio,args_extra=None):
            ...
            for count,heat in enumerate(heatTGS):
                ...
                instante_captura=datetime.now()
                RSTGS = ((Modulacion.Vcc*MPID.Rl_2600)/(valueTGS2600/1000.0)) - MPID.Rl_2600
                Modulacion.queue.put(["Muestras_iniciales",count,heat,valueTGS2600,RSTGS,heat,heat,instante_captura,[4]])
                print("Da time: ",Modulacion.SLEEP - (time.time() - time_ini))
                time.sleep(Modulacion.SLEEP - (time.time() - time_ini))
            super().cerrar_electrovalvulas()
            return False
        ...
        #def capturas_muestras_iniciales(self,samplesinicio,args_extra=None):
        #        super().capturas_muestras_iniciales(samplesinicio)
        #        self.lastError,self.addError,self.temp = 0,0,args_extra[1]
        #        self.captura_odorante(Modulacion.capturas_iniciales,[4],samplesinicio,"Muestras_iniciales",args_extra[0],self.lastError,
        #                self.addError,self.temp,args_extra[2],args_extra[3],args_extra[4],args_extra[5],args_extra[6],args_extra[7],args_extra[8])
